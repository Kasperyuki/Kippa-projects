...
def img_down_custom(bday,hour,minute,img_count):
    ...
    print(bday[6:8])
    time = datetime(2021, 1, int(bday[6:8]))
    #day = int(bday[6:8])
    #hour = int(bday[4:6])
    #minute = int(bday[6:8])
    #hour = 0
    ...
    for i in range(int(img_count)):
        #hour = int(bday[4:6])
        #minute = int(bday[6:8])
        time = datetime(2021, 1, int(int(bday[6:8])), hour, minute)
        ...
        hour_txt = hours[hour]
        min_txt = mins[minute]
        url_end = f"{day_txt}{hour_txt}{min_txt}"
        url_day = f"{url_img[0:98]}{url_end}{url_img[-4:-1]}D"
        #url_all = url_text.split(" ")
        #url_img = url_all[6][5:][:-1]
        print(url_day)
